...
const CourseCard = ({ course }) => {
  ...
  return (
    ...
    <Link to={`/courses/${id}/`}>
      <img className='shadow-md rounded-lg mb-3 hover:-translate-y-1 duration-200' src={image} alt="Courses Image" />
      <div className=' flex flex-col gap-2'>
        <h2 className='font-bold text-lg'>{title}</h2>
        {/* <p className='font-semibold'>{description}</p> */}
        <p className='flex gap-2 text-[#0369A1]'><IoBookOutline className='mt-[5px] text-[#0369A1]'/> {course_modules} Modules</p>
